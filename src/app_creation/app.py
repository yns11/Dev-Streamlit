...
def reinitialiser_wizard() -> None:
    for cle in ("etape", "donnees", "pages", "frs_connus", "uploader_key",
...
# =====================================================================
    ...
    photo = st.file_uploader(
        "Photographier / choisir une page", type=["jpg", "jpeg", "png"],
    ...
    if photo is not None:
        ...
        try:
            with st.spinner("Traitement de la page…"):
                page_traitee = images.scanner_document(photo.getvalue(), mode)
            st.image(page_traitee, caption=f"Aperçu — {mode}", use_column_width=True)
            with st.expander("Voir la photo originale"):
                st.image(photo.getvalue(), use_column_width=True)

            if st.button("➕ Empiler cette page", type="primary", use_container_width=True):
                st.session_state.pages.append(page_traitee)
                st.session_state.uploader_key += 1  # réinitialise le widget → pas de double ajout
